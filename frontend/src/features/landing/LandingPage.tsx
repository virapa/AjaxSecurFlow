    ...
    return (
        <div className="min-h-screen bg-[#020617] text-white selection:bg-blue-500/30">
            ...
            <header className="fixed top-0 z-50 w-full border-b border-white/5 bg-[#020617]/80 backdrop-blur-md">
                <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
                    <div className="flex items-center gap-2">
                        <div className="h-8 w-8 rounded-lg bg-blue-600 flex items-center justify-center font-bold text-white shadow-lg shadow-blue-500/20">A</div>
                        <span className="text-xl font-bold tracking-tight">AjaxSecurFlow</span>
                    </div>
            ...
            <section className="relative flex flex-col items-center justify-center overflow-hidden pt-32 pb-20 px-4 text-center">
                <div className="absolute top-[-10%] left-1/2 -z-10 h-[500px] w-[800px] -translate-x-1/2 rounded-[100%] bg-blue-500/10 blur-[120px]" />
                <div className="absolute bottom-0 left-1/2 -z-10 h-[300px] w-full -translate-x-1/2 bg-gradient-to-t from-blue-600/5 to-transparent" />

                <div className="inline-flex items-center gap-2 rounded-full border border-blue-500/20 bg-blue-500/5 px-3 py-1 text-[10px] font-medium tracking-widest text-blue-400 uppercase">
                ...
                <div className="mt-10 flex flex-col sm:flex-row gap-4 items-center">
                    <Link href="/login">
                        <Button className="h-12 px-8 text-sm font-bold shadow-xl shadow-blue-500/20 group">
            ...
            <section id="features" className="max-w-7xl mx-auto py-24 px-4 sm:px-6 lg:px-8">
                <div className="mb-16">
                    <h2 className="text-xs font-bold uppercase tracking-widest text-blue-500 mb-2">{t.landing.capabilities.tag}</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {t.landing.capabilities.items.map((feature, idx) => (
                        <div key={idx} className="group relative rounded-2xl border border-white/5 bg-white/[0.02] p-8 transition-all hover:bg-white/[0.04]">
                            <div className="mb-6 flex h-10 w-10 items-center justify-center rounded-lg bg-blue-600/10 text-xl group-hover:bg-blue-600/20 transition-colors">
                                {feature.icon}
                            </div>
            ...
            {/* Security Section */}
            <section id="security" className="max-w-7xl mx-auto py-24 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-transparent via-blue-600/5 to-transparent">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                    <div className="grid grid-cols-2 gap-4">
                        <div className="h-48 rounded-2xl bg-gradient-to-br from-blue-600/20 to-transparent border border-white/5 flex items-center justify-center text-4xl">📁</div>
                        ...
                        <div className="h-48 rounded-2xl bg-gradient-to-tl from-blue-600/20 to-transparent border border-white/5 flex items-center justify-center text-4xl">🛡️</div>
                    </div>

                    <div>
                        <h2 className="text-4xl font-extrabold tracking-tight mb-6">
                            {t.landing.security.title} <span className="text-blue-500">{t.landing.security.titleHighlight}</span> {t.landing.security.titleSuffix}
                        </h2>
                        ...
                        <div className="space-y-8">
                            {t.landing.security.items.map((item, idx) => (
                                <div key={idx} className="flex gap-4">
                                    <div className="flex-shrink-0 h-6 w-6 rounded-full bg-blue-600/20 flex items-center justify-center text-xs text-blue-500">✓</div>
                                    <div>
            ...
            <section id="pricing" className="max-w-7xl mx-auto py-24 px-4 sm:px-6 lg:px-8 text-center">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 max-w-7xl mx-auto">
                    {t.landing.pricing.plans.map((plan, idx) => (
                        ...
                        >
                            {plan.recommended && (
                                <div className="absolute top-0 right-1/2 translate-x-1/2 -translate-y-1/2 bg-blue-500 text-[10px] font-bold uppercase tracking-widest px-4 py-1.5 rounded-full shadow-lg whitespace-nowrap">
                                    {plan.recommended}
                                </div>
                            )}

                            <span className={`text-xs font-bold uppercase tracking-wider ${plan.recommended ? 'text-blue-400' : 'text-gray-500'}`}>
                                {plan.name}
                            </span>
                            <div className="mt-4 flex items-baseline gap-1">
                                <span className={`text-4xl font-extrabold ${plan.recommended ? 'text-blue-400' : 'text-white'}`}>{plan.price}</span>
                            ...
                            <ul className="space-y-4 mb-8">
                                {plan.features.map((feat, i) => (
                                    <li key={i} className={`flex items-center gap-3 text-sm ${plan.recommended ? 'text-blue-400' : 'text-gray-400'}`}>
                ...
                <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start gap-12">
                    <div className="max-w-xs">
                        <div className="flex items-center gap-2 mb-4">
                            <div className="h-6 w-6 rounded bg-blue-600 flex items-center justify-center font-bold text-xs">A</div>
                            <span className="font-bold">AjaxSecurFlow</span>
                        </div>
