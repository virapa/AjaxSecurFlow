...
export const UpgradePrompt: React.FC<UpgradePromptProps> = ({
    ...
    if (compact) {
        return (
            <div className={`flex items-center justify-between p-4 bg-white/[0.02] border border-white/10 rounded-xl ${className}`}>
                ...
                <Link href="/billing">
                    <button className="px-4 py-2 bg-cyan-600 hover:bg-cyan-500 text-white text-xs font-bold rounded-lg transition-colors">
                        Actualizar
                    </button>
                </Link>
            </div>
    ...
    return (
        <div className={`relative block w-full min-h-[400px] overflow-hidden ${className}`} style={{ width: '100%', minHeight: '400px' }}>
            ...
            <div className="absolute inset-0 backdrop-blur-xl bg-cyan-950/40 z-10" />

            {/* Content */}
            <div className="absolute inset-0 z-20 flex items-center justify-center p-8 overflow-y-auto">
                <div className="max-w-md w-full bg-cyan-900/40 border-2 border-cyan-400/50 rounded-3xl p-10 text-center shadow-2xl shadow-cyan-500/20 ring-1 ring-white/20 backdrop-blur-md">
                    {/* Lock Icon */}
                    ...
                    {/* CTA Button */}
                    <Link href="/billing">
                        <button className="w-full px-6 py-3 bg-gradient-to-r from-cyan-600 to-purple-600 hover:from-cyan-500 hover:to-purple-500 text-white font-bold rounded-xl transition-all shadow-lg shadow-cyan-500/20">
                            Actualizar ahora
                        </button>
                    </Link>
