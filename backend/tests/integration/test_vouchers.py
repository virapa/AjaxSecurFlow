...
async def test_voucher_flow(async_client: AsyncClient, mock_user_subscription):
    ...
    with patch("backend.app.modules.billing.service.redeem_voucher", new_callable=AsyncMock) as mock_redeem:
        mock_redeem.return_value = False
        response = await async_client.post(
            f"{settings.API_V1_STR}/billing/redeem",
    ...
    response = await async_client.post(
        f"{settings.API_V1_STR}/billing/generate",
        params={"duration_days": 30, "count": 1}
    )
